...
def sample_noise(batch_size, dim, seed=None):
    ...
    noise = torch.rand(batch_size, dim)*2 - 1 #rand: (0,1)
    return noise
...
def generator_loss(logits_fake):
    ...
    # We want the discriminator predict fake images as true! 
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = 1/2 * (scores_real - 1).pow(2) + 1/2 * (scores_fake).pow(2)
...
def build_dc_classifier(batch_size):
    ...
    model = nn.Sequential(
        ...
        nn.MaxPool2d(2, stride=2),
        Flatten(),
        nn.Linear(1024, 1024), # 4*4*64 = 1024
        nn.LeakyReLU(0.01, inplace=True),
        nn.Linear(1024, 1)
...
def build_dc_generator(noise_dim=NOISE_DIM):
    ...
    model = nn.Sequential(
        ...
        nn.ReLU(inplace=True),
        nn.BatchNorm1d(6272),
        # Reshape 7*7*128 to 7,7,128
        Unflatten(128, C=128, H=7, W=7), # batch_size = 128
