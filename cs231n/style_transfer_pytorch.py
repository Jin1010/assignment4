...
def content_loss(content_weight, content_current, content_original):
    ...
    content_current = content_current.reshape(1*C_l, H_l*W_l)
    content_original = content_original.reshape(1*C_l, H_l*W_l)
    loss = content_weight * torch.sum((content_current - content_original).pow(2))
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
        # For each layer, compute gram matrix
        style_current = gram_matrix(feats[style_layers[i]]) 
        # Compute loss
        style_loss += style_weights[i] * torch.sum((style_current - style_targets[i]).pow(2))
...
def tv_loss(img, tv_weight):
    ...
    # For H pair: (2-1)...(H-(H-1))
    # img[:, :, 1:, :] : 2...H   img[:, :, :-1, :] : 1...H-1
